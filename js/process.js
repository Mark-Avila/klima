...
//set weather map ----------------------
let weatherMap = L.map("weathermap", {
  minZoom: 2,
  maxZoom: 12,
});
...
let map = L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
  attribution:
    'Map data &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors, Imagery © <a href="https://www.openweathermap.org/">OpenWeatherMap</a>',
}).addTo(weatherMap);

let owm = L.tileLayer(
  `https://tile.openweathermap.org/map/precipitation/{z}/{x}/{y}.png?appid=${process.env.OWM}`
).addTo(weatherMap);
...
async function fetchLocation(query) {
  let res = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q='+query+'&units=metric&appid=${process.env.OWM}`
  );
...
async function fetchForecastData(lat, lon) {
  let res = await fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=minutely&units=metric&appid=e1558fe0d8dcc08923d8122663466af2`
  );
...
function displayForecastData(data) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    $(`#day-${i + 1}-temp`).text(
      `${Math.round(data.daily[i].temp.max)} / ${Math.round(
...
function getWeatherBg(id) {
  if (id >= 200 && id <= 232) {
    ...
    console.error("Error: weather id not found");
  }
}
...
function displayMap(layer) {
  ...
  let owm = L.tileLayer(
    `https://tile.openweathermap.org/map/${layer}/{z}/{x}/{y}.png?appid=e1558fe0d8dcc08923d8122663466af2`
  ).addTo(weatherMap);
}
...
async function openModal(location, daynum) {
  ...
  let locale = await fetch(
    "https://api.openweathermap.org/data/2.5/weather?q=" +
      location +
      "&units=metric&appid=e1558fe0d8dcc08923d8122663466af2"
  );
  let locdata = await locale.json();

  let weather = await fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${locdata.coord.lat}&lon=${locdata.coord.lon}&exclude=current,minutely,hourly&units=metric&appid=e1558fe0d8dcc08923d8122663466af2`
  );
  ...
  $("#di-temp").text(
    `${Math.round(data.daily[daynum].temp.max)} / ${Math.round(
      data.daily[daynum].temp.min
    )}°C`
  );
